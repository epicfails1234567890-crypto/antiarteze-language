...
ejecutarVentana=(xsize,ysize)=>{
    const psCommand = `Add-Type -AssemblyName System.Windows.Forms; $form = New-Object System.Windows.Forms.Form; $form.Text = 'Mi Ventana'; $form.Size = New-Object System.Drawing.Size(${xsize},${ysize}); $form.Add_Paint({ $g = $_.Graphics; $brush = New-Object System.Drawing.SolidBrush([System.Drawing.Color]::Green); }); $form.ShowDialog()`;
...
function transformarSintaxis(codigo) {
    ...
    return codigo.split('\n').map(linea => {
        ...
        if (l.startsWith("window")) {
            return l.replace(/window(?:\s+(\d+)\s+(\d+))?/, (match, ancho, alto) => {
                // Si no se pasan argumentos, usa 400 por defecto
                const w = ancho || 400;
                const h = alto || 400;
                return `ejecutarVentana(${w}, ${h});`;
            });
        }
