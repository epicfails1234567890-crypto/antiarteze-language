...
ejecutarVentana = (xsize, ysize, wname, px, py, pwidth, pheight) => {
    ...
    const psCommand = `
        ...
        $form.Text = '${wname.replace(/'/g, "''")}'; 
        $form.Size = New-Object System.Drawing.Size(${xsize},${ysize}); 
        
        $form.Add_Paint({
            param($s, $e);
            $e.Graphics.FillRectangle([System.Drawing.Brushes]::Green, ${px}, ${py}, ${pwidth}, ${pheight});
        });
...
function transformarSintaxis(codigo) {
    ...
    return codigo.split('\n').map(linea => {
        ...
        // El Regex ahora espera: window ancho alto "nombre" x y w h
        if (l.startsWith("window")) {
            return l.replace(/window\s+(\d+)\s+(\d+)\s+"([^"]+)"\s+(\d+)\s+(\d+)\s+(\d+)\s+(\d+)/, 
            (match, ancho, alto, nombre, x, y, w_rect, h_rect) => {
                
                // Aquí llamas a tu función con todos los parámetros
                // Puedes pasar el nombre y las coordenadas a ejecutarVentana
                return `ejecutarVentana(${ancho}, ${alto}, '${nombre}', ${x}, ${y}, ${w_rect}, ${h_rect});`;
            });
        }
...
window 640 480 "hola mundo" 64 64 64 64
